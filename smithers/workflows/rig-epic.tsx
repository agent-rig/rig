...
const WORKFLOWS_DIR = new URL(".", import.meta.url).pathname;
const RIG_TASK_REF = {
  path: resolve(WORKFLOWS_DIR, "rig-task.tsx"),
  approvedRoot: resolve(WORKFLOWS_DIR, ".."),
};
 ...
 *          Subflow(rig-task --base <integration-branch>) -> merge-gate      (phase run|full)
...
const epicResultSchema = z.object({
  phase: z.string(),
...
// Mirror of rig-task's designated result — the shape Subflow persists per child.
...
const { Workflow, Sequence, Parallel, Task, Branch, Approval, UI, smithers, outputs } = createSmithers({
  input: inputSchema,
...
export default smithers((ctx) => {
  const { phase, merge, advisor } = ctx.input;
  ...
  const doPlan = (phase === "plan" || isFull) && ctx.input.feature.trim() !== "";
  ...
  const pre = ctx.outputMaybe(outputs.epicPreflight, { nodeId: "epic-preflight" });
  const planRow = ctx.outputMaybe(outputs.plan, { nodeId: "plan" });
  const startRow = ctx.outputMaybe(outputs.start, { nodeId: "start" });
  ...
  const merged = (c: Child) => c.status === "merged" || ctx.outputMaybe(outputs.merge, { nodeId: `merge-${c.id}` })?.merged === true;
  ...
  // gate on ONE approval, then run children with their own spec gate OFF. A child
  // must never pause mid-run — a paused Subflow child fails the whole epic.
  const specArch = ctx.outputMaybe(outputs.epicSpec, { nodeId: "epic-spec-architect" });
  ...
  const specDir = ctx.outputMaybe(outputs.specDirection, { nodeId: "spec-direction" });
  ...
  const consolidated = ctx.outputMaybe(outputs.reviewConsolidated, { nodeId: "review-consolidate" });
  const reviewClean = consolidated?.clean === true;
  const reviewApproved = ctx.outputMaybe(outputs.reviewApproval, { nodeId: "approve-review" })?.approved === true;
  ...
  const finishApproved = isFull ? ctx.outputMaybe(outputs.finishApproval, { nodeId: "approve-finish" })?.approved === true : true;
  ...
  return (
    <Workflow name="rig-epic">
      ...
      <Task id="epic-preflight" output={outputs.epicPreflight} retries={0}>
        {async () => {
          ...
          try {
            ...
            for (const dir of epicDirs) {
              ...
              // parent may be the ticket id `CEX-553` or the branch slug `cex-553-…`). Never
              ...
              const needle = ctx.input.parent.trim().toLowerCase();
              const pick = needle
          ...
          if (integrationBranch === "" && ctx.input.parent.trim() !== "") {
            integrationBranch = ctx.input.parent.trim();
            source = "input-parent";
          }
          const banner = `rig-epic: ${ctx.input.phase} — ${integrationBranch ? `epic ${integrationBranch}` : ctx.input.feature || "(new epic)"}. PRs target ${integrationBranch || "the integration branch"}, not the trunk. Will not auto-merge the trunk without --merge.`;
          return {
      ...
      {doPlan ? (
        <Sequence>
          <Task id="plan" agent={ROLE.architect} output={outputs.plan} deps={{ "epic-preflight": outputs.epicPreflight }}>
            {(d) => {
              const p = d["epic-preflight"];
              const namedParent = (ctx.input.parent || p.parent || "").trim();
...
Return: parent (id or slug), parentTitle, integrationBranch, whyEpic, and childrenJson = a JSON array string of {id, title, blockedBy:[...], status}.`;
          ...
          <Task id="start" agent={ROLE.coord} output={outputs.start} deps={{ plan: outputs.plan, "epic-preflight": outputs.epicPreflight }}>
            {(d) => {
              const pl = d["plan"];
              const p = d["epic-preflight"];
...
Return integrationBranch, the stateFile path, and a summary. Do NOT start executing children — that is an explicit opt-in.`;
      ...
      {/* ── front-loaded spec review of ALL children, then ONE approval ── */}
      {specReviewReady ? (
        <Sequence>
          <Parallel id="epic-spec">
            <Task id="epic-spec-architect" agent={ROLE.architect} output={outputs.epicSpec} deps={{ "epic-preflight": outputs.epicPreflight }}>
            ...
            <Task id="epic-spec-qa" agent={ROLE.reviewer} output={outputs.epicSpec} deps={{ "epic-preflight": outputs.epicPreflight }}>
          ...
          {specReviewed && !specAnswered ? (
            advisor ? (
              // Autonomous arch gate: Fable decides proceed/direction in the human's
              // place so a kicked-off epic never parks here. Same node id + schema as
              // the HumanTask, so everything downstream (runApproved, per-child
              // specNotes, the proceed=false halt) is unchanged.
              <Task id="spec-direction" agent={ROLE.advisor} output={outputs.specDirection}>
                {() => `You are the ARCH ADVISOR for the epic on \`${integrationBranch}\` (${SKILL}). ${cd} You stand in for the human at the front-loaded spec gate: BEFORE ${children.length} parallel implementation runs (${children.map((c) => c.id).join(", ")}) commit against the integration branch, you decide whether to proceed and produce the steering direction every child's coder will follow. No human is waiting — you ARE the gate.
      ...
      {reviewReady ? (
        <Sequence>
          <Parallel id="review-lenses">
            <Task id="review-simplify" agent={ROLE.architect} output={outputs.reviewLens} deps={{ "epic-preflight": outputs.epicPreflight }}>
...
Find abstractions to collapse, helpers one PR added that another PR's final shape made redundant, config knobs nobody sets, code paths the combined diff made dead, one-caller types. Concrete deletions/merges with file:line, highest-impact first. Skip correctness. Return lens="simplify", counts p0p1/p2, and findings.`}
            ...
            <Task id="review-crosspr" agent={ROLE.reviewer} output={outputs.reviewLens} deps={{ "epic-preflight": outputs.epicPreflight }}>
...
Per-PR review already ran; catch interactions only visible at the merged shape (PR-A's helper vs PR-D's stale caller; PR-B removed a knob PR-F still reads). Return lens="crosspr", counts p0p1/p2, and findings with file:line + category.`}
            ...
            <Task id="review-deadcode" agent={ROLE.reviewer} output={outputs.reviewLens} deps={{ "epic-preflight": outputs.epicPreflight }}>
          ...
          <Task id="review-consolidate" agent={ROLE.reviewer} output={outputs.reviewConsolidated} dependsOn={["review-simplify", "review-crosspr", "review-deadcode"]}>
          ...
          {consolidated && !consolidated.clean && reviewApproved ? (
            <Task id="review-fix" agent={ROLE.coder} output={outputs.reviewFix} deps={{ "review-consolidate": outputs.reviewConsolidated }}>
...
Return a summary of what you changed.`}
      ...
      {finishReady ? (
        <Task id="finish-squash" agent={ROLE.coord} output={outputs.squashPr} deps={{ "epic-preflight": outputs.epicPreflight }}>
          {(d) => {
            const p = d["epic-preflight"];
...
Return the PR number and url.`;
      ...
      {/* ── final report ── */}
      <Task id="epic-result" output={outputs.epicResult} retries={0} dependsOn={["epic-preflight"]}>
        {() => {
          const sq = ctx.outputMaybe(outputs.squashPr, { nodeId: "finish-squash" });
          const st = ctx.outputMaybe(outputs.start, { nodeId: "start" });
